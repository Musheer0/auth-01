  ...
  try {
    ...
    try {
      ...
      try {
        ...
        const html_template = generateOtpEmail({
        ...
        const email_verificatio_token = await createEmailVerificationToken(
          new_user.id,
          otp,
          prisma,
        );
        await SendEmail(
          html_template,
          new_user.primary_email,
          'Verifiy your email',
        );
        return {
          verify_token: email_verificatio_token.tokenId,
          expiresAt: email_verificatio_token.expiresAt,
        };
      } catch (error) {
